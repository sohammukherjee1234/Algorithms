...
/* Infix to Prefix helper methods */
...
int main() {
    ...
    stack prefix = newStack(prefix); // stack for holding prefix term in reverse order
    int i;
    char ch;
    printf("Enter Infix expression:\n");
    scanf("%s", infix);
    printf("Prefix expression:\n");
    for (i = strlen(infix) - 1; i >= 0; i--) { // scanning infix expression from right
        ch = infix[i];
        if (ch == '+' || ch == '-' || ch == '*' || ch == '/' || ch == '^') { // if character is an operator
            while ( ( ! isEmpty(s) ) && prec(ch) <= prec( top(s) ) ) {
                    prefix = push(prefix, top(s)); // push the top element of stack into prefix stack
            ...
            prefix = push(prefix, ch); // push the character into prefix stack
        }
    }
    while ( (! isEmpty(s)) ) {
        if (top(s) != '(' && top(s) != ')') prefix = push(prefix, top(s));
        s = pop(s);
    }
